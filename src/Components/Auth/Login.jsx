...
export default function Login() {
  ...
  const roles = ['Admin', 'Agent', 'DistributorMain'];
  ...
  useEffect(() => {
    setCurrentIndex(roles.indexOf(role));
  }, [role]);
    ...
    <>
      {user ? (
        <div>
          ...
          <CoinCount user={user} role={role} email={email} app={app} />
        </div>
      ) : (
                      ...
                      <form className="space-y-4 lg:space-y-6" onSubmit={(e) => {
                        ...
                        <p className="flex flex-row gap-10">
                          {roles.map((role, index) => (
                            <a onClick={() => setCurrentIndex(index)} key={index} className={`flex flex-row ${currentIndex === index ? 'hidden' : 'font-bold text-blue-500'}`}>
                              {role} ?
                            </a>
                          ))}
                        </p>
