...
export default function Login() {
    ...
    const handleLogin = async () => {
        try {
            ...
            const userRole = determineUserRole(user.uid);

            console.log(userRole);

            // Assuming selectedRole is the expected role for the login
            if (userRole !== selectedRole) {
                // If the user's role doesn't match the expected role, sign them out
                await signOut(auth);
                console.log("User signed out due to role mismatch");
                // You can redirect or handle the situation as needed
            } else {
                // User's role matches the expected role, proceed with the application logic
                console.log("User role matches the expected role");
                navigate('/dashboard');
                // You can redirect or handle the situation as needed
            }
        } catch (error) {
        ...
        // Function to determine the user's role based on UID
        function determineUserRole(uid) {
            const roles = ["Admin", "Agent", "Distributer"];

            for (const role of roles) {
                if (data[role] && data[role][uid]) {
                    return role;
                }
            }
